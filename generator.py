...
for i in range(num_loans):
    ...
    book_chosen = random.choice(borrowers)
    if date_returned > datetime.now():
        date_returned = "NULL"
    else :
        book_chosen["state"] = "Available"
    loan = {
        "id": i + 1,
        "book id": book_chosen["id"],
        "borrower id": random.choice(borrowers)["id"],
        "due date": due_date,
...
base_query_books = "INSERT INTO Books ( title, author, ISBN, published_date, genre, shelf_location, state) VALUES "
values_books = ', '.join(f"('{book['title']}', '{book['author']}', '{book['ISBN']}', '{book['published_date']}', '{book['genre']}', '{book['location']}', '{book['state']}')" for book in books)
data_books = base_query_books + values_books + ";"

base_query_borrowers = "INSERT INTO Borrower ( first_name, last_name, email, date_of_birth, membership_date) VALUES "
values_borrowers = ', '.join(f"( '{borrower['first name']}', '{borrower['last name']}', '{borrower['email']}', '{borrower['date of birth']}', '{borrower['mambership  date']}')" for borrower in borrowers)
...
values_loans = ', '.join(f"('{loan['book id']}', '{loan['borrower id']}', '{loan['due date']}', '{loan['date borrower']}', '{loan['date returned']}')" for loan in loans)
